#!/bin/bash
#SBATCH --job-name=4dct_lilly
#SBATCH --output=/home/li5273/Desktop/data/output/2026/0820/lilly/slurm_%j.log
#SBATCH --nodes=1
...
mkdir -p /home/li5273/Desktop/data/output/2026/0820/lilly

# ── Verify / extract the dataset ─────────────────────────────────────────────
# Lilly_recon.py requires an already-extracted dataset directory (it checks
# os.path.isdir on --data_path), and the demo_data copy has previously been
# left partially extracted (unstable storage during copy), so verify file
# counts before trusting it and re-extract straight from the /depot source
# if incomplete. Safe to run concurrently with dev_recon_job.sh's extraction
# step: extraction is staged then atomically moved into place.
...
python Lilly_recon.py \
  ...
  --output_path /home/li5273/Desktop/data/output/2026/0820/lilly \
  --max_mace_itr 10
